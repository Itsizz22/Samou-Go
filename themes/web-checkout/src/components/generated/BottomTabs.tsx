  ...
  return <nav className="fixed inset-x-0 bottom-0 z-50 bg-surface/95 backdrop-blur border-t border-line shadow-raised px-4 safe-bottom">
      <div className="flex items-center justify-around h-16 max-w-lg mx-auto">
        ...
        return <button key={tab.id} onClick={() => {
          if (onTabChange) { onTabChange(tab.id); return; }
          const paths = { home: '/home', explore: '/search', orders: '/orders', profile: '/profile' };
          window.location.assign(buildSsoUrl(`${appUrl('customer')}${paths[tab.id]}`));
        }} aria-current={isActive ? 'page' : undefined} className={`flex flex-col items-center justify-center w-full h-full transition-colors rounded-xl active:scale-95 focus:outline-none focus:ring-2 focus:ring-brand/40 ${isActive ? 'text-brand' : 'text-ink-muted hover:text-ink-soft'}`}>
