...
export function SamouGoCaptain() {
  ...
  // Captain pickup-handoff flow state. The code is the shared secret between
  // the store (which visibly prints it) and the captain who must recite it
  // back to verify the handover when claiming a READY_FOR_PICKUP order.
    ...
    <div className="mt-3 rounded-xl border border-line bg-canvas p-3">
      ...
      {currentZone && !justSet && (
        <p className="mb-2 rounded-lg bg-brand-tint px-2.5 py-1.5 text-micro font-semibold text-brand-deep">
          ...
          {t(currentZone.nameAr, currentZone.nameEn)} — {currentZone.deliveryFee} ₪
        </p>
      )}
