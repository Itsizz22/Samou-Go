...
export function FeaturedProductsShowcase({ products, loading, onAdd }: { products: PopularProduct[]; loading: boolean; onAdd: (product: PopularProduct) => void }) {
  ...
  return <section className="mx-auto max-w-md px-5 pt-6" aria-label={t('أطباق مميزة اخترناها لك', 'Featured dishes picked for you')} aria-busy={loading}>
    ...
    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
      {loading && !products.length ? [0, 1].map(key => <div key={key} className="skeleton h-80 rounded-2xl" aria-hidden="true" />) : products.slice(0, expanded ? products.length : 20).map(product => <DishCard key={product.id} product={product} onAdd={onAdd} />)}
    </div>
    {products.length > 20 && <button type="button" onClick={() => setExpanded(value => !value)} aria-expanded={expanded} className="mt-3 min-h-11 w-full rounded-xl border border-line bg-surface text-sm font-bold text-brand focus-visible:ring-2 focus-visible:ring-brand">{expanded ? t('عرض أقل', 'Show less') : t('شاهد جميع الأطباق المميزة', 'See all featured dishes')}</button>}
  </section>;
}
