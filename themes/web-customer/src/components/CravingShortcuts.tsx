...
export function CravingShortcuts({ products }: { products: Product[] }) {
  const { t } = useLanguage();
  return <section className="my-6" aria-labelledby="cravings-title">
    <h2 id="cravings-title" className="mb-3 text-lg font-extrabold text-ink">{t('ماذا تشتهي اليوم؟', 'What are you craving?')}</h2>
    <div className="flex gap-3 overflow-x-auto pb-2" data-swipe-back="off">
      {cravings.map(({ ar, en, match, icon: Icon }) => {
        ...
        return <Link key={ar} to={`/search?q=${encodeURIComponent(ar)}`} className="group flex w-20 shrink-0 flex-col items-center gap-2 rounded-2xl p-1 text-center focus-visible:outline-2 focus-visible:outline-brand">
          <span className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl bg-brand-tint text-brand-dark transition-transform motion-safe:group-hover:scale-105">
