    ...
    <div className="mx-auto max-w-lg">
      <div className="overflow-hidden rounded-2xl border border-line bg-surface shadow-card">
        ...
        <div className="space-y-5 p-4 sm:p-5">
          ...
          <section className="space-y-2 rounded-2xl border border-line bg-canvas p-4"><label className="block font-bold">{t('منطقة المتجر', 'Store area')}<AppSelect aria-label={t("منطقة المتجر", "Store area")} className="input-field mt-2 w-full" value={form.deliveryZoneId} onChange={e => update('deliveryZoneId', e.target.value)} disabled={zones.loading || !!zones.error}><option value="">{t('اختر منطقة المتجر', 'Select store area')}</option>{form.deliveryZoneId && !zones.data?.some(z => z.id === form.deliveryZoneId) && <option value={form.deliveryZoneId}>المنطقة الحالية غير متاحة — اختر منطقة نشطة</option>}{zones.data?.map(z => <option key={z.id} value={z.id}>{isArabic ? z.nameAr : z.nameEn}</option>)}</AppSelect></label><p className="text-xs text-ink-muted">{t('نفس المناطق التي يختار منها الزبون؛ تُستخدم لحساب التوصيل من المتجر إلى عنوانه.', 'These are the same customer areas, used to calculate store-to-customer delivery.')}</p>{zones.error && <button type="button" onClick={() => void zones.reload()} className="min-h-11 text-brand">تعذر تحميل المناطق — إعادة المحاولة</button>}</section>
