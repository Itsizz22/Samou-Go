      ...
      {!stores.error && (stores.loading || featured.length > 0) && <section className="mx-auto max-w-md px-5 pt-8" aria-labelledby="featured-title" aria-busy={stores.loading}>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2" dir="rtl">
          {stores.loading
            ...
            : featured.map(({ store, category, initials, gradient }) => <article key={store.id} className="min-w-[196px] overflow-hidden rounded-2xl bg-surface shadow-card"><div className={`relative flex h-24 items-center justify-center bg-gradient-to-br ${gradient}`}>{store.logoUrl ? <img src={store.logoUrl} alt="" className="h-full w-full object-cover" loading="lazy" /> : <span className="text-3xl font-black text-white/40">{initials}</span>}<span className={`absolute bottom-2 start-2 rounded-full px-2 py-1 text-[10px] font-bold ${store.isActive ? 'bg-surface text-brand-dark' : 'bg-canvas text-ink-muted'}`}>{store.isActive ? <>مفتوح <span dir="ltr">Open</span></> : <>مغلق <span dir="ltr">Closed</span></>}</span><button type="button" aria-label={`Favorite ${store.nameEn}`} aria-pressed={liked.includes(store.id)} onClick={() => toggleLike(store.id)} className="absolute end-2 top-2 rounded-full bg-surface/85 p-1.5 text-brand"><Heart size={15} fill={liked.includes(store.id) ? 'currentColor' : 'none'} /></button></div><div className="p-3 text-end"><h3 className="truncate text-sm font-extrabold">{store.nameAr}</h3><p className="mt-0.5 truncate text-[11px] text-ink-muted" dir="ltr">{store.nameEn}</p><p className="mt-2 text-[10px] text-ink-muted">{category.ar}</p><div className="mt-2 flex items-center justify-between gap-2"><span className="flex items-center gap-1"><span className="text-[10px] text-ink-subtle">من</span><DeliveryFee amount={baseFee} variant="badge" showIcon /></span><span className="truncate text-[10px] text-ink-muted" dir="ltr">{category.en}</span></div></div></article>)}
        </div>
      </section>}
      ...
      {!stores.error && (stores.loading || cards.length > 0) && <section className="mx-auto max-w-md px-5 pt-8" aria-labelledby="nearby-title" aria-busy={stores.loading}>
        ...
        <div className="space-y-3">
          {stores.loading
            ...
            : cards.map(({ store, category, initials, tint }) => <article key={store.id} className="flex items-center gap-3 rounded-2xl bg-surface p-3 shadow-card"><div className={`flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl text-sm font-black ${tint}`}>{store.logoUrl ? <img src={store.logoUrl} alt="" className="h-full w-full object-cover" loading="lazy" /> : initials}</div><div className="min-w-0 flex-1 text-end"><h3 className="truncate text-sm font-extrabold">{store.nameAr}</h3><p className="truncate text-[11px] text-ink-muted" dir="ltr">{store.nameEn} · {category.en}</p><p className="mt-1 flex items-center gap-2 text-[10px] font-semibold text-ink-muted"><span>يبدأ من</span><DeliveryFee amount={baseFee} variant="inline" /></p></div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-bold ${store.isActive ? 'bg-brand-tint text-brand-dark' : 'bg-canvas text-ink-muted'}`}>{store.isActive ? 'مفتوح' : 'مغلق'}</span></article>)}
        </div>
      </section>}
