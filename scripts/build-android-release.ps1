param([int]$VersionCode = 38, [string]$VersionName = '1.0.37', [string]$JdkHome = (Join-Path $env:USERPROFILE '.jdks/jbr-21.0.11'))
...
$firebaseClient = @($firebaseConfig.client | Where-Object { $_.client_info.android_client_info.package_name -eq 'com.samougo.customer' })
if ($firebaseConfig.project_info.project_id -ne 'samou-go' -or $firebaseClient.Count -ne 1) { throw 'Firebase configuration must target samou-go / com.samougo.customer.' }
if ($VersionCode -lt 12 -or [string]::IsNullOrWhiteSpace($VersionName)) { throw 'Use versionCode 12 or higher and a nonempty versionName.' }
