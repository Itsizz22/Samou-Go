...
Do not activate until credentials, sender approval and credit are available. If the provider restricts IPs, allow Render outbound addresses through the provider account. No real SMS has been sent by this verification.

POST JSON to the fixed HTTPS SendJSON.aspx endpoint, one recipient, mode 0, URI-encoded message. Only the exact documented success STATUS counts as gateway acceptance. It does not prove handset delivery. No automatic retry on uncertain network delivery. No raw provider body or credentials logged. Ten-second timeout and redirects disabled.
