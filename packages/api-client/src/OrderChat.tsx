...
import { ApiError, getToken, subscribeTokenChange, getOrderChat, getOrderChatPeers, markOrderChatRead, sendOrderChat } from './api';
...
function Conversation({ orderId }: { orderId: string }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      catch (e) { if (!controller.signal.aborted) { setError(e instanceof Error ? e.message : 'تعذر تحميل المحادثات'); if (e instanceof ApiError && [401, 403, 404].includes(e.status)) { setBlocked(true); setOverview(null); } } }
  ...
  return <section ref={root} dir="rtl" className="my-3 rounded-2xl border border-line bg-surface p-4 text-start text-ink" aria-label="محادثات الطلب">
    ...
    {open && <>
      ...
      {blocked && <button type="button" className="min-h-11 text-sm font-bold text-brand" onClick={() => { setError(''); setBlocked(false); }}>إعادة المحاولة</button>}
...
function Thread({ orderId, peerId, closed, visible, onRead }: { orderId: string; peerId: string; closed: boolean; visible: boolean; onRead: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch (e) { if (!controller.signal.aborted) { setError(e instanceof Error ? e.message : 'تعذر تحميل الرسائل'); setLoading(false); if (e instanceof ApiError && [401, 403, 404].includes(e.status)) { setBlocked(true); setMessages([]); setBefore(null); knownIds.current.clear(); pagingStarted.current = false; } } }
      finally { running = false; }
    };
    void load(); const timer = window.setInterval(() => void load(), 4000); document.addEventListener('visibilitychange', load);
    return () => { controller.abort(); window.clearInterval(timer); document.removeEventListener('visibilitychange', load); };
  }, [orderId, peerId, visible, blocked]);
  const send = async () => {
    if (busy || blocked || closed || !draft.trim()) return;
  ...
  return <div className="mt-3 space-y-3">
    <div ref={scroll} className="max-h-80 space-y-3 overflow-y-auto rounded-xl bg-canvas p-3" aria-label="الرسائل">
      ...
      {loading ? <p role="status">جارٍ التحميل…</p> : messages.length === 0 ? (error ? null : <p className="py-4 text-center text-sm text-ink-muted">ابدأ المحادثة بخصوص طلبك</p>) : messages.map(m => <div key={m.id} className={`max-w-[90%] rounded-xl p-3 text-sm ${m.senderId === peerId ? 'me-auto border border-line bg-surface' : 'ms-auto bg-brand text-white'}`}>
    ...
    {blocked ? <button type="button" className="min-h-11 text-sm font-bold text-brand" onClick={() => { setError(''); setLoading(true); setBlocked(false); }}>إعادة المحاولة</button> : closed ? <p className="text-sm text-ink-muted">انتهى الطلب. يمكنك قراءة المحادثة السابقة.</p> : <form onSubmit={e => { e.preventDefault(); void send(); }} className="space-y-2">
