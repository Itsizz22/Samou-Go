...
export async function uploadImage(
  ...
  const compressed = input.kind === 'video' || input.kind === 'audio' || input.kind === 'store' ? file : await compressImage(file);
...
export function searchProducts(search = '', page = 1, signal?: AbortSignal, dishesOnly = false, dishSection: 'all' | 'discovery' | 'featured' = 'all', options: { storeId?: string; foodStoresOnly?: boolean; shuffleSeed?: string } = {}): Promise<{ items: import('@samou-go/shared-types').PopularProduct[]; total: number; page: number; pageSize: number }> {
  return request('GET', '/stores/search-products', { query: { search, page: String(page), dishesOnly: String(dishesOnly), dishSection, shuffleSeed: options.shuffleSeed, storeId: options.storeId, foodStoresOnly: String(options.foodStoresOnly ?? false) }, signal });
}
