...
export async function captainPoolScope(captainId: string): Promise<Prisma.OrderWhereInput> {
  const captain = await prisma.user.findUnique({ where: { id: captainId }, select: { role: true, isActive: true, isVerified: true, isAvailable: true, assignedStoreId: true } });
  ...
  return {
    ...
    ...(captain.assignedStoreId ? { storeId: captain.assignedStoreId } : {}),
    store: { OR: [{ dedicatedCaptains: { none: {} } }, { dedicatedCaptains: { some: { id: captainId } } }] },
  };
}

export async function eligibleCaptainIds(storeId: string): Promise<string[]> {
  const store = await prisma.store.findUnique({ where: { id: storeId }, select: { dedicatedCaptains: { select: { id: true } } } });
  if (!store) return [];
  const dedicated = store.dedicatedCaptains.map(c => c.id);
  const users = await prisma.user.findMany({ where: {
    role: UserRole.CAPTAIN, isActive: true, isVerified: true, isAvailable: true,
    OR: [{ assignedStoreId: null }, { assignedStoreId: storeId }],
