...
export function OrderCustomerDetails({ order, showDestination = false, showStore = true }: { order: Pick<OrderSummary, 'customerContact' | 'deliveryDestination' | 'storeContact'>; showDestination?: boolean; showStore?: boolean }) {
  const contact = order.customerContact;
  const destination = order.deliveryDestination;
  if (!contact && !(showStore && order.storeContact) && !(showDestination && destination)) return null;
  return <section dir="rtl" aria-label="بيانات الزبون" className="my-3 space-y-2 rounded-xl border border-line bg-canvas p-3 text-start text-sm text-ink">
    ...
    {showStore && order.storeContact && <OrderContactCard title="المتجر" contact={order.storeContact} />}
