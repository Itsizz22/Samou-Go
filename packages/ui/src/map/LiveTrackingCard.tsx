...
import { BaseMap, validMapPoint, type MapMarker } from './BaseMap';
export function LiveTrackingCard({ orderId, load }: { orderId: string; load: (id: string, signal?: AbortSignal) => Promise<LiveOrderTracking> }) {
  ...
  const stale = error || !data?.location || !Number.isFinite(Date.parse(data.location.updatedAt)) || !validMapPoint([data.location.lat, data.location.lng]) || Date.parse(data.location.updatedAt) > now + 5000 || now - Date.parse(data.location.updatedAt) > 60000 || data.stale;
  ...
  const meters = route?.distanceMeters ?? null;
  const distanceLabel = 'مسافة الطريق';
  const routeNotice = route ? `وصول تقديري خلال ${Math.max(1, Math.ceil(route.durationSeconds / 60))} دقيقة · لا يشمل حركة المرور أو وقت التحضير` : 'مسار الشوارع غير متاح حاليًا؛ لا تتوفر مسافة طريق أو مدة وصول موثوقة.';
  const markers: MapMarker[] = data ? [
    ...(data.store ? [{ id: 'store', kind: 'store' as const, position: [data.store.lat, data.store.lng] as [number, number], label: data.store.label }] : []),
    ...(data.customer ?? (data.stage === 'customer' ? data.destination : null) ? [{ id: 'customer', kind: 'destination' as const, position: [(data.customer ?? data.destination)!.lat, (data.customer ?? data.destination)!.lng] as [number, number], label: 'موقع التسليم' }] : []),
    ...(data.location ? [{ id: 'captain', kind: 'captain' as const, stale, position: [data.location.lat, data.location.lng] as [number, number], label: stale ? 'آخر موقع معروف للكابتن' : 'الكابتن' }] : []),
  ].filter(marker => validMapPoint(marker.position)) : [];
  return <section dir="rtl" className="overflow-hidden rounded-2xl border border-line bg-surface shadow-card">
    <div className="space-y-2 p-4"><div className="flex items-center justify-between gap-3"><h3 className="font-bold">تتبع التوصيل</h3><button type="button" className="rounded-xl border border-line px-3 py-2 text-sm text-brand" onClick={() => setRetry(n => n + 1)}>تحديث</button></div>
      ...
      {data && <>
        ...
        <p className="text-sm">{data.addressNote}</p><p className="text-sm">{data.zone ? data.zone + ' · ' : ''}{data.address}</p>
    ...
    {data?.enabled && markers[0] && data?.stage !== 'complete' && <div className="px-3 pb-3">
      ...
      <BaseMap center={markers[0].position} markers={markers} fitKey={`${orderId}:${data.stage}`} route={route?.coordinates} className="h-72 w-full rounded-2xl" />
    </div>}
    {expanded && <dialog ref={dialogRef} onCancel={() => setExpanded(false)} aria-label="خريطة تتبع التوصيل" dir="rtl" style={{ width: '100vw', maxWidth: 'none', height: '100dvh', maxHeight: 'none', margin: 0, padding: 0, border: 0 }} className="bg-surface text-ink">
      <div className="flex h-full flex-col">
        ...
        <div className="relative min-h-0 flex-1">
          {data?.enabled && data.stage !== 'complete' && markers[0] ? <BaseMap center={markers[0].position} markers={markers} fitKey={`${orderId}:${data.stage}`} route={route?.coordinates} className="h-full w-full" /> : <p className="p-6">التتبع غير متاح لهذا الطلب حاليًا</p>}
        </div>
