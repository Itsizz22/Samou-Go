...
- **`@samou-go/shared-types` builds on `npm install`** (prepare script). The API (CJS) needs `dist/`; web apps resolve TS source via the `import` condition. After editing shared-types: `npm run build` (root = shared-types + api) or `npm run build:all` for everything.
- **Each theme's `src/lib/delivery.ts` re-exports `@samou-go/shared-types`** (single source of truth). Fee wording/tariff changes are made once in shared-types and picked up by all 7 themes — never duplicate the fee logic into a theme again.
